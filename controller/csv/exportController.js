...
exports.chef_csv = (req, res) => {
  DB.query('SELECT * FROM chef', null, (rows, err) => {
    try {
      let data = JSON.parse(JSON.stringify(rows));
      var header = ['Chef ID', 'Chef Name', 'Chef Contact', 'Chef Brigade'];
...
exports.invoice_csv = (req, res) => {
  DB.query(
    'SELECT invoice_id, customer_name, customer_no, customer_email, payment_method, order_total FROM invoice INNER JOIN customer',
    null,
    (rows, err) => {
      try {
        let data = JSON.parse(JSON.stringify(rows));
